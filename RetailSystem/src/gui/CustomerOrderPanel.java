...
public class CustomerOrderPanel extends JPanel {
	...
	public void displayProductsTable(String productName) {
		if (productName == "") {
			populateAvailableProductsArayOfObjectsForAllProducts();
			// Display only for selected product
		} else {
			populateAvailableProductsArrayOfObjectsForASpecifficProduct(productName);
		}
		String columnNames[] = { "Id", "Name", "Supplier", "Category", "Price",
				"Discounted?", "Quantity", "Amount to Order" };
		TableModelWithLastColEditable productsTableModel = new TableModelWithLastColEditable(
				availableProductsArray, columnNames);
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					String choice = JOptionPane.showInputDialog(
							CustomerOrderPanel.this, "Enter the amount");
					try {
						if (choice != null) {
		...
		tableAvailableProducts.getColumnModel().getSelectionModel()
				.addListSelectionListener(new ListSelectionListener() {
					...
					public void valueChanged(ListSelectionEvent e) {
						...
						tableAvailableProducts.changeSelection(row, 7, false,
								false);
					}
				});
		// tableAvailableProducts.setComponentPopupMenu(new
		// OrderTableJPopupMenu());
		scrollPane.getViewport().add(tableAvailableProducts);
		scrollPane.repaint();

	}// end displayProductsTable

	private void populateAvailableProductsArayOfObjectsForAllProducts() {
	...
	private void populateAvailableProductsArrayOfObjectsForASpecifficProduct(
			String productName) {
		int size = 0;
		for (Product prod : Shop.getProducts()) {
			if (prod.getName().equalsIgnoreCase(productName)
					&& prod.isAvailable() == true && prod.isDeleted() == false) {
				size++;
			}
		}
		...
		DecimalFormat df = new DecimalFormat("#.00");
		for (Product product : Shop.getProducts()) {
			if (product.isAvailable() == true && product.isDeleted() == false
	...
	public void displayOrderTable(boolean forCustomerOnly, int stockOrderID) {
		...
		// CustomeOrder array, otherwise only for the selected customer
		if (stockOrderID != 0) {
			populateArrayOrdersOfObjectsForASpecificOrder(stockOrderID, currentCustomerOrders, columnNames1);
		} else if (forCustomerOnly == false) {
			populateArrayOrdersOfObjectsForAllOrders(currentCustomerOrders, columnNames1);
		} else {
	...
	private void populateArrayordersOfObjectsForSelectedCustomer(ArrayList<CustomerOrder> currentCustomerOrders, String[] columnNames1) {
		...
		for (int j = 0; j < thisCustomerOrders.size(); j++) {
			...
			arrayOrders[j][1] = thisCustomerOrders.get(j).getCustomer()
					.getCustomerFName()
					+ " "
					+ thisCustomerOrders.get(j).getCustomer()
							.getCustomerLName();
			arrayOrders[j][2] = thisCustomerOrders.get(j).getStaff()
					.getName()
					+ " "
					+ thisCustomerOrders.get(j).getStaff().getSurname();
			arrayOrders[j][3] = sdf.format(thisCustomerOrders.get(j)
					.getCreationDate());
			arrayOrders[j][4] = df.format(thisCustomerOrders.get(j)
					.getTotalNet());
			arrayOrders[j][5] = df.format(thisCustomerOrders.get(j)
					.getTotalGross());
	...
	private void populateArrayOrdersOfObjectsForASpecificOrder(int stockOrderID, ArrayList<CustomerOrder> currentCustomerOrders, String columnNames1[]) {
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm");
		DecimalFormat df = new DecimalFormat("#.00");
		...
		for (CustomerOrder custOrder : currentCustomerOrders) {
			if (stockOrderID == custOrder.getId()) {
				...
				arrayOrders[0][1] = custOrder.getCustomer()
						.getCustomerFName()
						+ " "
						+ custOrder.getCustomer().getCustomerLName();
				arrayOrders[0][2] = custOrder.getStaff().getName() + " "
						+ custOrder.getStaff().getSurname();
	...
	private void populateArrayOrdersOfObjectsForAllOrders(ArrayList<CustomerOrder> currentCustomerOrders,String[] columnNames1) {
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm");
		DecimalFormat df = new DecimalFormat("#.00");
		...
		for (int i = 0; i < currentCustomerOrders.size(); i++) {
			...
			arrayOrders[i][1] = currentCustomerOrders.get(i).getCustomer()
					.getCustomerFName()
					+ " "
					+ currentCustomerOrders.get(i).getCustomer()
							.getCustomerLName();
			arrayOrders[i][2] = currentCustomerOrders.get(i).getStaff()
					.getName()
					+ " "
					+ currentCustomerOrders.get(i).getStaff().getSurname();
			arrayOrders[i][3] = sdf.format(currentCustomerOrders.get(i)
					.getCreationDate());
			arrayOrders[i][4] = df.format(currentCustomerOrders.get(i)
					.getTotalNet());
			arrayOrders[i][5] = df.format(currentCustomerOrders.get(i)
					.getTotalGross());
	...
	/**
	 * Returns ArrayList of CustomerOrder for the currently selected customer.
	 * If none found a message is displayed and null returned.
	 * 
	 * @return
	 */
	...
	/**
	 * Decrements the amount from product quantity. Products as well as the
	 * Object[][] array (availableProductsArray) are modified
	 * 
	 * @param productId
	...
	public boolean decrementProductQuantityFromProducts(int productId,
			int deductableAmount) {
	...
	private boolean changeOrderCompletionStatus(int orderID) {
		...
		if (Shop.getCustomerOrders().get(getOrderIndex(orderID)).isComplete() == false) {
			...
			displayErrorMessage("Order already completed. Nothing to update.",
					Color.RED);
			return false;
		}
	}
	...
	private void updateOrderStatusInTable(boolean isOrderCompleted, int row) {
		if (isOrderCompleted == true) {
			AbstractTableModel model = (AbstractTableModel) tableOrders
					.getModel();
		...
		public OrderTableJPopupMenu() {
			...
			menuItemUpdate.addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent e) {
					...
					TableModelWithLastColEditable model = (TableModelWithLastColEditable) tableOrders
							.getModel();
					model.setValueAt(true, row, 6);
					model.fireTableDataChanged();
			...
			menuItemShow.addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent e) {
					...
					htmlText += "<tr><td>" + id + "</td></td>" + customer
							+ "</td><td>" + staff + "</td><td>" + date
							+ "</td><td>" + totalNet + "</td><td>" + totalGross
							+ "</td></tr>";
	...
	public void setSelectedCustomer() {
		Customer customer = getCustomerFromConcatenatedName(comboSelectCustomer
				.getSelectedItem().toString());
		if (customer != null) {
			selectedCustomer = customer;
			lblActiveCustomer.setText(selectedCustomer.getCustomerFName() + " "
					+ selectedCustomer.getCustomerLName());
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (int i = 0; i < availableProductsArray.length; i++) {
				if ((int) availableProductsArray[i][7] > 0
						&& (int) availableProductsArray[i][6] >= (int) availableProductsArray[i][7]) {
					...
					for (Supplier sup : Shop.getSuppliers()) {
						if (sup.getSupplierName().equalsIgnoreCase(
								(String) availableProductsArray[i][2])) {
					...
					Object[] p = { id, name, supplier, category, markupPrice,
							discounted, quantity, amount };
					...
					for (Object[] o : arrayCurrentOrder) {
						if (id == (int) o[0]) {
							...
							// check whether the total amount for this product
							// exceeds the product's quantity
							...
							if ((int) o[7] + amount > quantityOfProduct) {
								JOptionPane
										.showMessageDialog(
												CustomerOrderPanel.this,
												"The total amount for "
														+ (String) o[1]
														+ " would exceed the available quantity: "
														+ quantity
					...
					found = true;
					decrementProductAvalableQuantity(
							(int) availableProductsArray[i][0],
							(int) availableProductsArray[i][7]);
					displayErrorMessage("Product(s) added to order", Color.blue);
					AbstractTableModel model = (AbstractTableModel) tableAvailableProducts
							.getModel();
					model.fireTableDataChanged();
				} else if ((int) availableProductsArray[i][7] > 0
						&& (int) availableProductsArray[i][6] < (int) availableProductsArray[i][7]) {
					displayErrorMessage(
							"You cannot order more than available!", Color.red);
				}
			}
			if (found == false) {
	...
	/**
	 * This handler is called when user clicks on Submit Order button. If
	 * selected customer is not null, then user is prompted to submit the order.
	 * If user confirms, the table changes are compared with
	 * availableProductArray to find if all product amounts can be purchased.
	 * Only then is the order created and the table redrawn to reflect changes
	 * in product quantity. Product quantity is also adjusted
	 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (dialog.getValue() == 1) {
				...
				for (Object[] x : arrayCurrentOrder) {
					// check whether the available quantity is less than the
					// amount entered
					if ((Integer) x[6] < (Integer) x[7]) {
						displayErrorMessage(
								"You cannot order higher amount than currently available",
								Color.red);
						return;
					}
					int id = (Integer) x[0];
					...
					if (supplierFound == false) {
						JOptionPane
								.showMessageDialog(
										CustomerOrderPanel.this,
										"No supplier has been found for at least on of the products. Make sure you do not edit suppliers and make the order in the same time.");
						return;
					}
					...
					productsToOrder.add(new ProductToOrder(id, name,
							tempSupplier, category, price, discounted, amount));
				}

				// create the actual order
				// check if the user did not remove manually all products from
				// table
				if (productsToOrder.size() == 0) {
					displayErrorMessage("Nothing to order!", Color.red);
					return;
				} else {
					CustomerOrder order = new CustomerOrder(selectedCustomer,
							GUIBackBone.getLoggedStaffMember(), productsToOrder);
					order.setId(Shop.getCustomerOrders()
							.get(Shop.getCustomerOrders().size() - 1).getId() + 1);
					...
					for (ProductToOrder x : productsToOrder) {
						...
						decrementProductQuantityFromProducts(x.getId(),
								x.getAmount());
					}
					AbstractTableModel model = (AbstractTableModel) tableAvailableProducts
							.getModel();
	 ...
	 */
	public class ButtonDisplayOrdersForAllCustomersHandler implements
			ActionListener {
